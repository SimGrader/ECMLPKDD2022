...
def cal_acc(score, sl):
    score2 = []
    for i in range(len(score)):
        if score["score"][i] > sl[0]:
            score2.append(4)
        elif score["score"][i] >= sl[1] and score["score"][i] < sl[0]:
            score2.append(3)
        elif score["score"][i] >= sl[2] and score["score"][i] < sl[1]:
            score2.append(2)
        elif score["score"][i] < sl[2] and score["score"][i] > sl[3]:
            score2.append(1)
        else:
            score2.append(0)
    y_true = list(score["manul"])
    from sklearn import metrics

    acc = metrics.accuracy_score(score2, y_true)
    f1 = metrics.f1_score(y_true, score2, labels=[0, 1, 2, 3, 4], average="weighted")
    r = metrics.recall_score(y_true, score2, labels=[0, 1, 2, 3, 4], average="weighted")
    P = metrics.precision_score(
        y_true, score2, labels=[0, 1, 2, 3, 4], average="weighted"
    )
    print(acc, f1, r, P)
