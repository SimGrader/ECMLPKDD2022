...
def train_MLP(feature, label):
    ...
    for i in range(5):
        ...
        # train_x, test_x, train_y, test_y = train_test_split(feature, label, train_size=0.8, shuffle=True)
...
def train_LinearRegression(feature, label):
    train_x, train_y, test_x, test_y = get_k_fold_data(5, 1, feature, label)
    ...
    # y_test_pred = np.argmax(y_test_pred, axis=1)
    # acc = metrics.accuracy_score(y_test_pred, test_y)
    print("LinearRegression result:", acc)
    # return y_pred
...
def train_Lasso(feature, label):
    ...
    for i in range(5):
        ...
        # y_test_pred = lasso.fit(train_x, train_y).predict(test_x)
        # acc = metrics.accuracy_score(y_test_pred, test_y)
...
if __name__ == "__main__":
    ...
    if modelstr == "MLP":
        ...
        y_pred = train_Lasso(feature, label)
    elif modelstr == "bayes":
        y_pred = train_bayes(feature, label)
    else:
